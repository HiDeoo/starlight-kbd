...
When no keyboard type has the `default` property set to `true`, the plugin will automatically detect the user's operating system on their first visit and select the appropriate keyboard type. The plugin looks for type IDs that match: 'mac' for macOS, 'windows' for Windows, and 'linux' for Linux systems.
...
You can also omit the `default` property to enable automatic OS detection:
...
With this configuration, the plugin will automatically detect the user's operating system and select the appropriate keyboard type.
...
Only one keyboard type can be set as default. If no default is specified, the plugin will automatically detect the user's operating system and select the appropriate keyboard type if available (e.g., 'mac', 'windows', or 'linux'). If autodetection fails or no matching type is found, the first keyboard type in the list will be used.
